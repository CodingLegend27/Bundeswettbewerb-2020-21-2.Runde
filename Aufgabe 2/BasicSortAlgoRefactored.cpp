...
int main() {
	...
	// neue Adjazenzliste, in der zueinander abhängige Sorten aufgelistet werden
	vector<set<int> > dependencies(MAXN);

	// vector<set<int> > sorte2schale(MAXN);

	vvi sorte2schale(MAXN);

	for (int sorte = 1; sorte < MAXN; ++sorte) {
		// alle möglichen Schalen ermitteln
		set<int> possibleSchalen;
		
		for (int schale = 1; schale < MAXN; ++schale) {
			if (adj[sorte][schale] == maxVObstsorten[sorte]) {
				possibleSchalen.insert(schale);
			}
		}
 		...
 		// 0: schale geht nicht
 		// 1: schale geht vielleicht (mit Konkurrenz)
 		// 2: schale geht sicher (eindeutig)
 		vi isPossibleSchale(MAXN, -1);

		for (int schale : possibleSchalen) {

			bool schaleSicher = true;

			// über jede andere Sorte wird iteriert
			for (int i = 1; i < MAXN; ++i) {

				// falls gleiche Sorte oder anderer maxV-Value
				// Sorte ist keine Konkurrenz --> Sorte wird übersprungen
				if (i == sorte or maxVObstsorten[i] != maxVObstsorten[sorte]) {
	
					// if (maxVObstsorten[i] > maxVObstsorten[sorte]) {
					// 	// Schale gehört der anderen Sorte, da diese einen höheren maxV-Wert hat
					// 	isPossibleSchale[schale] = 0;
				...
				// falls die andere Sorte eine Konkurrenz zur aktuellen Sorte ist
				if (adj[i][schale] == maxVObstsorten[i] or adj[i][schale] > adj[sorte][schale]) {
					schaleSicher = false;
					isPossibleSchale[schale] = 1;

					dependencies[sorte].insert(i);
			...
			if (schaleSicher) {
				// schale geht sicher
				isPossibleSchale[schale] = 2;
			}
		}

		sorte2schale[sorte] = isPossibleSchale;

		// if (!schaleWorks) {
		// 	// nicht eindeutig
		// 	sorte2schale[sorte] = possibleSchalen;
		// }
	}

	for (int i = 1; i < MAXN; ++i) {
		cout << num2words[i] << " abhängig von insg. "<<dependencies[i].size()<<" anderen Sorten:\n";
		...
		cout << "Nummern der Schalen: \n";
		for (int j = 1; j < MAXN; ++j) {
			cout << sorte2schale[i][j] << " ";
		}
		cout <<"\n\n";
	}
	...
	// Berechne Anzahl möglicher Schalen pro Sorte
	vi numSchalen(MAXN, 0);
	// for (int w : wunsche) {
	for (int w = 1; w < MAXN; ++w) {
		int counter = 0;

		for (int i = 1; i < MAXN; ++i) {
			if (sorte2schale[w][i] > 0) counter++;
		}

		numSchalen[w] = counter;
	}
	...
	for (int w = 1; w < MAXN; ++w) {
		...
		if (dependencies[w].empty()) {
			cout << "> hat keine Konkurrenz von anderen Sorten\n";
		} else {
			cout << "> hat die Konkurrenz/folgende Sorten müssen zuerst bestimmt werden: ";
			for (int next : dependencies[w]) cout << num2words[next] << " ";
			cout << "\n";
		}

		if (numSchalen[w] == 1) {
			cout << ">> ist sicher in Schale Nr. ";
			for (int i = 1; i < MAXN; ++i) if (sorte2schale[w][i] == 2) cout << i;
		}
		else if (numSchalen[w] == 0) {
			cout << ">> kann keiner Schale zugewiesen werden\n";
			
			works = false;
		}
		else {
			// Sorte kann sich in mehreren Schalen befinden
			cout << ">> kann sich in folgenden Schalen befinden: x(e) = eindeutig; x(k) = nicht eindeutig/mit Konkurrenz;\n";
			cout << ">>> ";
			for (int i = 1; i < MAXN; ++i) {
				if (sorte2schale[w][i] == 1) {
					cout << i << "(k) ";
				} else if (sorte2schale[w][i] == 2) {
					cout << i << "(e) ";
				}
			}
			cout << "\n"; 
	...
	// 	if (dependencies[w].empty()) {
	// 		cout << "Sorte " << num2words[w] << " eindeutig -> in Schale Nr. " << *sorte2schale[w].begin() << "\n\n";
	// 		continue;
	// 	}

	// 	// mindestens eine störende Sorte -> Schüsseln können nicht eindeutig bestimmt werden
	// 	works = false;

	// 	cout << "Sorte " << num2words[w] << " kann in den Schalen Nr. ";
	// 	for (int s : sorte2schale[w]) cout << s << ", ";
	// 	cout << "sein\n";

	// 	cout << " > für die eindeutige Bestimmung müssen die Sorten: ";
	// 	for (int next : dependencies[w]) {
	// 		cout << num2words[next] << ", ";
	// 	}
	// 	cout << "bestimmt werden\n\n";
	// }
	...
	// // alle möglichen Verbindungen werden so sortiert, dass 
